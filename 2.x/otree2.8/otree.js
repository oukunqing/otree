...
function oTree(id, box, _cfg) {
    ...
    _.config = {
        ...
        //是否启用缓存回调
        isCacheCallback: _._checkBoolean([_cfg.isCacheCallback, _cfg.isCookieCallback], false),
        //缓存Cookie过期时长，单位：分钟
    ...
    //节点加载次数(批量加载)
    _.loadTimes = 0;
    //是否设置焦点，默认为设置，当动态按需加载时，不要设置焦点（仅设置选中就可以了）
    _.focused = true;
...
oTree.prototype._checkNodeData = function (data) {
    return !(typeof data != 'object' || data == null || data.id == undefined);
};
...
oTree.prototype.buildSwitch = function (id, isLast, showSwitch) {
    ...
    return ['<', this.strLinkLabel, ' href="javascript:;" tabIndex="-2" onmouseover="window.status=\'\';return true;"', this.strFocusHtml,
		' id="', this.buildId(this.objIdPrefix.actionlink, id), '"',
		' onclick="' + this.id + '.toggle(\'' + id + '\',this);"', this.strKeyEvent,
...
oTree.prototype.buildIcon = function (id, icon, iconOpen) {
    if (this.config.showIcon) {
        ...
        return ['<img src="', path, icon, '"',
			' lang="', icon, ',', iconOpen, '"',
			' id="', this.buildId(this.objIdPrefix.icon, id), '"',
			' onfocus="this.blur();" />'
        ].join('');
    }
    return '';
...
oTree.prototype.buildText = function (p) {
    var _ = this;
    var text = [];
    var tmp = ['<', _.strLinkLabel, ' id="', _.buildId(_.objIdPrefix.text, p.id), '" tabIndex="0"', ' lang="' + p.id + '"', _.strKeyEvent];
    text.push(tmp.join(''));

    var title = this.buildTitle(p.title);
    if (_.config.showTitle && title.length > 0) {
        text.push(' title="' + title + '"');
    }

    if (p.href != null) {
        tmp = [' class="link" href="', p.href, '"', (!_.config.showStatusText ? ' onmouseover="window.status=\'\';return true;"' : ''),
			(p.target != null ? ' target="' + p.target + '" ' : '')];
        text.push(tmp.join(''));
    } else {
        text.push(' class="link" href="javascript:;" onmouseover="window.status=\'\';return true;"');
    }
    text.push((p.dblclick ? ' ondblclick="' : ' onclick="') + _.id + '.clickText(\'' + p.id + '\', this);"');
    if (_.config.showContextMenu) {
        text.push(' oncontextmenu="' + _.id + '.contextmenu(event,\'' + p.id + '\', this);"');
    }
    if (_.config.keyEvent) {
        text.push(' onkeydown="' + _.id + '.keydown(event,\'' + p.id + '\', this);"');
    }

    tmp = [_.strFocusHtml, '>', p.name, '</', _.strLinkLabel, '>'];

    text.push(tmp.join(''));

    if (p.count >= 0) {
        text.push('<i id="' + _.buildId(_.objIdPrefix.count, p.id) + '" class="count">(' + p.count + ')</i>');
    }
    if (p.postfix != null) {
        text.push('<i id="' + _.buildId(_.objIdPrefix.postfix, p.id) + '" class="postfix">' + p.postfix + '</i>');
    }
    tmp = [];
...
oTree.prototype.buildCheckBox = function (p, nid) {
    ...
    return ['<input type="checkbox" id="', this.buildId(this.objIdPrefix.checkbox, p.id), '"',
		' name="', p.name, '" value="', (p.value || p.id), '"', ' lang="', p.id, '"',
		(p.checked ? ' checked="checked" ' : ''), (p.disabled ? ' disabled="disabled" ' : ''), this.strFocusHtml,
		' onclick="' + this.id + '.selectCheckBox(\'' + p.id + '\', ' + p.clickCheckedToggle + ', this, true);"', ' />'
    ].join('');
};

oTree.prototype.buildTitle = function (title) {
...
oTree.prototype.getIconPathName = function (src) {
    var pos = src.lastIndexOf('/') + 1;
    return { path: src.substr(0, pos), name: src.substr(pos) };
};

oTree.prototype.getIconRealPath = function (src) {
    if (!/\//g.test(src)) {
        return this.config.iconPath;
    } else if (!/^[\/]?(http)[s]?(:\/\/)/gi.test(src)) {
        var path = this.getIconPathName(src).path;
        var dir = this.config.iconPath;
        //清除结尾重叠的字符串
        //判断dir是否以path结尾
        if (dir.slice(-path.length) == path) {
            return dir.slice(0, dir.length - path.length);
        }
    }
    return '';
};
...
oTree.prototype.select = function (id, isCallback, _, isOpenTo) {
    _ = _ || this;
    if (typeof id == 'undefined') {
        ...
        if (txt != null) {
            ...
            if (_.focused && _.checkObjDisplay(_.selectedText)) {
            ...
            if (nData.id != undefined) {
                ...
                window.setTimeout(_.select, 128, id, isCallback, _, isOpenTo);
            }
        }
        ...
        if (node != null) {
            ...
            if (isOpenTo || typeof isOpenTo != 'boolean') {
                //展开当前节点的所有上级节点
                _.openTo(id, true);
            }
        }
...
oTree.prototype.getSelected = function () {
    return this.selectedText != null ? this.getNodeData(this.selectedText.lang) : '';
};
...
oTree.prototype.getChildNodes = function (pid) {
    var arr = [];
...
//将getChild函数改名为getChildNodes，保留getChild函数以便兼容
oTree.prototype.getChild = oTree.prototype.getChildNodes;
...
//更新节点内容、图标
oTree.prototype.update = function (p, append) {
    if (typeof p != 'object' || p.id == undefined) {
        return false;
    }
    var _ = this;
    var node = _.getNode(p.id);
    if (node != null) {
        var icon = _.getObj(_.objIdPrefix.icon, p.id);
        if (icon != null && p.icon != undefined) {
            icon.src = _.getIconRealPath(p.icon) + p.icon;
        }
        var txt = _.getObj(_.objIdPrefix.text, p.id);
        if (p.name != undefined) {
            txt.innerHTML = p.name;
        }

        if (typeof p.count == 'number' && p.count >= 0) {
            var count = _.getObj(_.objIdPrefix.count, p.id);
            if (count == null) {
                var oc = document.createElement('i');
                oc.id = _.buildId(_.objIdPrefix.count, p.id);
                oc.className = 'count';
                oc.innerHTML = '(' + p.count + ')';
                node.appendChild(oc);
            } else {
                postfix.innerHTML = p.postfix;
            }
        }

        if (typeof p.postfix == 'string') {
            var postfix = _.getObj(_.objIdPrefix.postfix, p.id);
            if (postfix == null) {
                var op = document.createElement('i');
                op.id = _.buildId(_.objIdPrefix.postfix, p.id);
                op.className = 'postfix';
                op.innerHTML = p.postfix;
                node.appendChild(op);
            } else {
                postfix.innerHTML = p.postfix;
            }
        ...
        if (_.config.showTitle) {
            node.title = _.buildTitle(p.title);
        }

        //更新完成后，重新保存节点数据
        p.iconOpen = p.iconOpen || p.icon;
        _.setNodeData(p.id, p, true);
    } else if (append) {
        _.add(p);
    }
};
...
oTree.prototype.selectCheckBox = function (id, isToggle, obj, isUserClick, isDisabled) {
    ...
    if (chb != null && chb.type == 'checkbox' && (!chb.disabled || this.config.selectDisabledCheckBox || isDisabled)) {
        //obj!=null表示点击的是复选框；当点击文字时，若不采用切换选中方式，则点击文字后始终为选中状态
...
oTree.prototype.openTo = function (id, isSelected) {
    var data = this._getNodeData(id);
    if (this._checkNodeData(data)) {
        var trees = (data.tree || '').split('|');
        for (var i = trees.length - 2; i >= 0; i--) {
            var pid = trees[i] || '';
            if (pid != '') {
                this.open(pid);
            }
        ...
        if (!isSelected || typeof isSelected != 'boolean') {
            this.select(id, false, this, false);
        }
    }
};

oTree.prototype.updateIcon = function (param, action) {
    if (typeof param != 'object') {
        return false;
    }
    switch (action || '') {
        case 'child':
            var pid = param.id || '', nodes = this.getChildNodes(pid), c = nodes.length;
            for (var i = 0; i < c; i++) {
                var node = nodes[i];
                var icon = this.getObj(this.objIdPrefix.icon, node.id);
                if (icon != null && param.icon != undefined) {
                    icon.src = this.getIconRealPath(param.icon) + param.icon;

                    //更新完成后，重新保存节点数据
                    this.setNodeData(node.id, { iconOpen: param.icon }, true);
                }
            }
            break;
        default:
            this.update(param, false);
            break;
    }
};
...
oTree.prototype.expandCache = function () {
    ...
    if (_.config.isCache) {
        ...
        if (value.length > 1 && value[1] != -1 && _.config.isCacheCallback) {
            _.isCacheCallback = true;
            _.select(value[1], true);
            _.isCacheCallback = false;
        }
...
oTree.prototype.findNode = function (field, value, isCallback) {
    if (typeof field != 'string' || (typeof value != 'string' && typeof value != 'number')) {
        return false;
    }
    var arr = this.arrNodeData || [];
    var node = null, nodeData = null;
    for (var i in arr) {
        var dr = arr[i];
        if (dr.isNode) {
            var content = '' + (field == 'code' ? (dr.code || '') : field == 'name' ? dr.name : dr.id);
            if (content.indexOf(value) >= 0) {
                node = this.getNode(dr.id);
                nodeData = dr;
                break;
            }
        }
