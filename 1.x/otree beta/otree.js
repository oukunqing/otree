...
oTree.prototype.root = function(param){
	...
	node.id = this.id + this.objIdPrefix.node + (param.id || 'root');
	node.lang = 'root';
	node.level = -1;
	node.className = 'node root';
	node.innerHTML = '<img src="' + this.config.imgPath + (param.icon || this.icon.root) + '" />' + (param.name || 'OTREE根节点');
...
oTree.prototype.add = function(param){
	var config = {
		...
		iconOpen: param.iconOpen || this.icon.folderOpen,
		checkbox: param.checkbox || param.checkBox || null,
		callback: param.callback || param.callBack || null,
		param: param.param || null,
		clickToggle: param.clickToggle || null, //点击文字是否打开/关闭子节点
...
oTree.prototype.create = function(param){
	var objParent = this.getNode(param.pid);
	if(param.pid <= 0 && objParent == null){
		objParent = this.obj;
	}
	if(objParent != null){
		var node = document.createElement('div');
	...
	if(!this.config.showLine && !this.config.showLineConfig){
		...
		for(var i=0; i<level; i++){
			space += '<img id="' + (this.id + 'line' + id + i) + '" name="' + (this.id + 'line') + '" lang="' + i + '"'
				+ 'src="' + this.config.imgPath + (this.config.showLine ? this.icon.line : this.icon.empty) + '" rel="' + this.icon.line + '" />';
		}
		return space;
	}
};
...
oTree.prototype.buildSwitch = function(id, isLast){
	...
	var strSwitch = '<img src="' + this.config.imgPath + strImgName + '"' 
	...
	strSwitch = '<a href="javascript:void(0);"' + this.showFocusHtml + ' tabIndex="-2" id="' + this.buildId(this.objIdPrefix.actionlink, id) + '">' + strSwitch + '</a>';
	return strSwitch;
};

oTree.prototype.buildIcon = function(id, icon, iconOpen){
	return !this.config.showIcon ? '' : '<img src="' + this.config.imgPath + icon + '" lang="' + icon + ',' + iconOpen + '" id="' + this.buildId(this.objIdPrefix.icon, id) + '" tabIndex="-2"' + ' onfocus="this.blur();" />';
};

oTree.prototype.buildText = function(param){
	var str = '<a id="' + this.buildId(this.objIdPrefix.text, param.id) + '"';
	if(param.href != null){
	...
	str	+= this.strFocusHtml + ' >' + param.name + (param.count >= 0 ? '(' + param.count + ')' : '') + '</a>';
	return str;
};
...
oTree.prototype.buildCheckBox = function(param, nid){
	if(param != null){
		...
		var str = '<input type="checkbox" id="' + this.buildId(this.objIdPrefix.checkbox, param.id) + '"'
			+ ' name="' + param.name + '" value="' + (param.value || param.id) + '"' 
...
oTree.prototype.setTextEvent = function(param, obj, callback){
	if(callback != null){
		...
		callback(param.param != null ? param.param : this.getTextParam(obj));
	} else {
		...
		if(obj != null && !obj.disabled){
			obj.onclick = function(){
				...
				if(param.callback != null){
					param.callback(param.param != null ? param.param : {chbName:param.name, id:param.id, value:this.value});
				}
			};
		}
	}
...
oTree.prototype.setSwitch = function(id, isOpen, isAppend){
	...
	if(a != null && (-1 == a.tabIndex || isAppend)){
		...
		if(!this.config.showLine && !this.config.showLineConfig){
			a.src = this.config.imgPath + (isOpen ? this.icon.minus : this.icon.plus);
		} else {
...
oTree.prototype.setLineAndSwitch = function(isHide, isAppend){
	...
	for(var i=0; i<arr.length; i++){
		...
		if(this.config.showLine){
			arr[i].src = this.config.imgPath + (isLast ? this.icon.empty : this.icon.line);
		}
...
oTree.prototype.showNodeLine = function(){
	...
	for(var i=0,c=arrLine.length; i<c; i++){
		arrLine[i].src = this.config.imgPath + arrLine[i].rel;
	}
	var arrSwitch = document.getElementsByName(this.id + 'switch');
	for(var i=0,c=arrSwitch.length; i<c; i++){
		arrSwitch[i].src = this.config.imgPath + arrSwitch[i].rel;
	}
};
...
oTree.prototype.removeSwitch = function(id){
	...
	if(a != null){
		if(this.config.showLine || this.config.showLineConfig){			
			...
			a.src = this.config.imgPath + this.icon.empty;
		}
		a.tabIndex = -2;
...
oTree.prototype.setIcon = function(id, isOpen, isAppend){	
	...
	if(icon != null && (-1 == icon.tabIndex || isAppend)){
		...
		icon.src = this.config.imgPath + (isOpen ? arrIcon[1] : arrIcon[0]);
		if(isAppend){
			icon.tabIndex = -1;
		}
	}
...
oTree.prototype.setTextParam = function(obj, param, isUpdate){
	...
	obj.lang = param.id + ',' + param.pid + ',' + param.type + ',' + param.code + ',' + param.name + ',' + param.level;
};
...
oTree.prototype.open = function(id){
	this.toggle(id, true);
};
...
oTree.prototype.openAll = function(){
	var arr = this.obj.childNodes;
	this.n = 0;
	this.toggleAll(arr, true);
};

oTree.prototype.closeAll = function(){
	var arr = this.obj.childNodes;
	this.n = 0;
	this.toggleAll(arr, false);
};
...
oTree.prototype.update = function(param){
	...
	if(node != null){
		...
		if(icon != null){
			icon.src = this.config.imgPath + param.icon;
			icon.lang = param.icon + ',' + param.icon;
		}
...
oTree.prototype.selectCheckBox = function(id, isChecked, obj, isDisabled){
	...
	if(chb != null && chb.type == 'checkbox' && (!chb.disabled || this.config.selectDisabledCheckBox || isDisabled)){
		...
		if(node != null){
			//设置子节点的checkbox
			this.setCheckBox(node, false, isChecked);
			//设置父节点的checkbox
			this.setCheckBox(node, true, isChecked);
		}
	}
};

oTree.prototype.setCheckBox = function(node, isParent, isChecked){
	if(isParent){
		...
		if(chbParent != null){
			...
			this.setCheckBox(parent, isParent, isChecked);
		}
	} else {
		...
		for(var i=0,c=childs.length; i<c; i++){
			if(this.getTagName(childs[i]) == this.nodeTag){
				...
				this.setCheckBox(childs[i], isParent, isChecked);
			}
		}
	}
};
...
oTree.prototype.getChecked = function(chbName, isDisabled){
	...
	var arrObj = [];
	if(chbName != undefined && chbName != null && chbName != ''){
		for(var i=0,c=arrChb.length; i<c; i++){
			if(arrChb[i].type == 'checkbox' && arrChb[i].name == chbName && arrChb[i].checked && (isDisabled == undefined || arrChb[i].disabled == isDisabled)){
				arrObj.push(arrChb[i]);
			}
		}
	} else {
		for(var i=0,c=arrChb.length; i<c; i++){
			if(arrChb[i].type == 'checkbox' && arrChb[i].checked && (isDisabled == undefined || arrChb[i].disabled == isDisabled)){
				arrObj.push(arrChb[i]);
			}
		}
	}
