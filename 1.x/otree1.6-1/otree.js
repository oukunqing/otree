...
oTree.prototype.setNodeData = function(id, p, isUpdate){
	if(isUpdate){
		...
		var _p = {
			...
			name: p.name || op.name,
			type: p.type || op.type,
			level: p.level || op.level,
			code: p.code || op.code,
			code: p.title || op.title,
			//rel: p.rel || op.rel,
			lang: p.id || op.lang,
			tree: p.tree || op.tree,
			display: p.display || op.display,
			loaded: p.loaded || op.loaded,
			showSwitch: p.showSwitch || op.showSwitch,
			icon: p.icon || op.icon,
			iconOpen: p.iconOpen || op.iconOpen,
			isExpand: p.isExpand || op.isExpand,
			isNode: op.isNode,
			hasChild: false
		};
