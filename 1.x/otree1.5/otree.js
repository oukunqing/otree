...
oTree.prototype.create = function(param, objParent){
	if(objParent != null){
		...
		node.isLoaded = param.loaded || false;
		...
		node.innerHTML = this.buildSpace(level, param.id) 
			+ this.buildSwitch(param.id, isLast, param.showSwitch) 
		...
		//强制显示打开/关闭按钮,单击节点动态添加子节点时用
		if(param.showSwitch){
			this.setSwitch(param.id, false, true);
		}
...
oTree.prototype.buildSwitch = function(id, isLast, showSwitch){
	...
	var strSwitch = '<img src="' + this.config.iconPath + strImgName + '"' 
		...
		+ ' lang="' + (showSwitch ? 1 : 0) + '"'
		+ ' class="expand-none" tabIndex="-2"' + this.strFocusHtml + ' />' ;
	strSwitch = '<' + this.strLinkLabel + ' href="javascript:void(0);"' + this.strFocusHtml + ' tabIndex="-2"'
...
oTree.prototype.setTextEvent = function(param, obj, callback){
	if(callback != null){
		...
		callback(param.param != null ? param.param : this.getTextParam(obj), this, this.callbackObj);
	} else {
		...
		if(obj != null && !obj.disabled){
			obj.onclick = function(){
				...
				if(param.callback != null){
					param.callback(param.param != null ? param.param : {chbName:param.name, id:param.id, value:this.value}, _, _.callbackObj);
				}
			};
		}
	}
			...
			obj.onclick = function(){
				...
				if(_.config.expandCallback != null && typeof _.config.expandCallback == 'function'){
					...
					if(node.rel == 'block'){
						...
						var param = txt != null ? _.getTextParam(txt) : {id:id, type:node.type, code:node.code}
						_.config.expandCallback(param, _);
					}
				}
			};
		}
...
oTree.prototype.setTextParam = function(obj, param, isUpdate){
	if(isUpdate){
		...
		param.loaded = param.loaded != undefined ? (param.loaded ? '1' : '0') : oldParam.loaded;
		param.tree = oldParam.tree;
	}
	obj.lang = param.id + ',' + param.pid + ',' + param.type + ',' + param.code + ',' + param.name + ',' + param.level + ',' + (param.loaded?'1':'0') + ',' + param.tree;
};
