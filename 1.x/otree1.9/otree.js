...
oTree.prototype.buildSwitch = function(id, isLast, showSwitch){
	...
	var arr = ['<', this.strLinkLabel, ' href="javascript:;" tabIndex="-2" onmouseover="window.status=\'\';return true;"', this.strFocusHtml,
		...
		(this.config.keyEvent ? ' onkeydown="' + this.id + '.keydown(\'' + id + '\', this);"' : ''),
		'>',
...
oTree.prototype.buildText = function(p){
	var strText = [];
	var arr = ['<', this.strLinkLabel, ' id="', this.buildId(this.objIdPrefix.text, p.id), '" tabIndex="0"', ' lang="' + p.id + '"', this.strKeyEvent];
	strText.push(arr.join(''));

	if(p.href != null){
		arr = [' class="link" href="', p.href, '"', (!this.config.showStatusText ? ' onmouseover="window.status=\'\';return true;"' : ''),
			(p.target != null ? ' target="' + p.target + '" ' : '')];
		strText.push(arr.join(''));
	} else {
		strText.push(' class="link" href="javascript:;" onmouseover="window.status=\'\';return true;"');
	}
	strText.push((p.dblclick ? ' ondblclick="' : ' onclick="') + this.id + '.clickText(\'' + p.id + '\', this);"');
	if(this.config.showContextMenu){
		strText.push(' oncontextmenu="' + this.id + '.contextmenu(\'' + p.id + '\', this);"');
	}
	if(this.config.keyEvent){
		strText.push(' onkeydown="' + this.id + '.keydown(\'' + p.id + '\', this);"');
	}

	arr = [this.strFocusHtml, '>', p.name, (p.count >= 0 ? '(' + p.count + ')' : ''), '</', this.strLinkLabel, '>'];
	strText.push(arr.join(''));
...
oTree.prototype.buildCheckBox = function(p, nid){
	...
	var arr = ['<input type="checkbox" id="', this.buildId(this.objIdPrefix.checkbox, p.id), '" name="', p.name, '" value="', (p.value || p.id),
		...
		' onclick="' + this.id + '.selectCheckBox(\'' + p.id + '\', null, this);"', ' />'];
	return arr.join('');
};
...
oTree.prototype.contextmenu = function(id, obj, p){
	if(p == undefined){
	...
	if(node != null){
		...
		while(sub != null){
			if(this.isNode(sub)){
				...
				this.setNodeDisplay(sub.lang, isExpand);
				count++;
			}
			sub = sub.nextSibling;
		}
		if(count > 0 || nData.showSwitch){
			this._setNodeExpandStatus(id, isExpand);
		}
		if(isExpand){
...
oTree.prototype.toggleAll = function(isOpen){
	for(var i in this.arrNodeData){
		...
		if(!nData.isRoot){
			...
			var hasChild = this.checkHasChild(id);
			if(hasChild){
				this._setNodeExpandStatus(id, isOpen);
			}
		}
	}
};
...
oTree.prototype.toggleLevel = function(level, isOpen){
	for(var i in this.arrNodeData){
		var nData = this.arrNodeData[i];
		if(!nData.isRoot){
			var id = nData.id;
			...
			if(nData.level <= level + 1){
				...
				if(hasChild){
					this._setNodeExpandStatus(id, nData.level <= level);
				}
			} else {
				if(nData.isDisplay){
					node = this.getNode(id);
...
oTree.prototype.selectCheckBox = function(id, isChecked, obj, isDisabled){
	...
	if(obj != null){	
		...
		if(checkbox != undefined){
			...
			if(typeof callback == 'function'){
				callback(checkbox.param != undefined ? checkbox.param : {chbName:checkbox.name, id:checkbox.id, value:obj.value}, this, this.callbackObj);
			}
		}
	}
};
...
oTree.prototype.toggleType = function(type, isOpen){
	for(var i in this.arrNodeData){
		var nData = this.arrNodeData[i];
		if(!nData.isRoot){
...
oTree.prototype.keydown = function(id, obj){
